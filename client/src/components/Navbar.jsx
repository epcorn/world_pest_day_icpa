...
const Navbar = () => {
  ...
  const NavLinkComponent = ({ to, text, icon: Icon }) => (
    <NavLink
      to={to}
  ...
  const MobileNavLink = ({ to, text, icon: Icon }) => (
    <NavLink
      ...
      onClick={() => setIsMobileMenuOpen(false)} // Close menu on click
      className={({ isActive }) =>
        `flex items-center space-x-3 w-full text-left px-6 py-3 transition-all duration-300 text-blue-100 hover:bg-blue-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-75 ${isActive ? "bg-blue-800 text-white font-bold" : "" // Active classes for mobile
        }`
      }
    >
      {Icon && <Icon size={22} />}
